...
- 2026-02-23: Keep tests aligned with actual URL policy. If `assertSafeHttpUrl` allows both `http` and `https`, tests must not assert `https`-only behavior; validate rejection only for non-HTTP schemes.
