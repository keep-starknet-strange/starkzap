...
![Cartridge controller integration hero](/assets/cartridge-controller.png)
...
You do not need to pass chain settings again when calling `sdk.connectCartridge()` or `sdk.onboard({ strategy: "cartridge" })`. Configure network once in `new StarkSDK(...)`, and the SDK forwards it.
