...
## Web vs Native Runtime

Starkzap supports two Cartridge integration paths:

- **Web** (`starkzap`): uses `@cartridge/controller` browser controller flow.
- **React Native** (`@starkzap/native`): uses a **native adapter** backed by controller.c browser-based sessions.

For React Native, Starkzap does **not** use `@cartridge/controller/session` (the web WASM session manager path). Native apps must register a Cartridge adapter once at app startup.

## StarkZap Integration Details

When StarkZap initializes Cartridge through this SDK, it forwards your SDK network config directly to Controller:

- `rpcUrl` -> `chains: [{ rpcUrl }]`
- `chainId` -> `defaultChainId`

This ensures the Controller session (and paymaster flow) is created on the same network your SDK uses.

You do not need to pass chain settings again when calling `sdk.connectCartridge()` or `sdk.onboard({ strategy: "cartridge" })`. Configure network once in `new StarkZap(...)`, and the SDK forwards it.

If session chain and SDK RPC chain diverge, sponsored flows can fail with errors such as:

- `"Account is not compatible with SNIP-9"`
- other paymaster/session mismatch errors

## Troubleshooting: "Account is not compatible with SNIP-9"

What it means: the active account/session cannot use the paymaster path for the selected chain/account type.

Options:

1. Fallback to `feeMode: "user_pays"` for that operation.
2. Use a Cartridge account/session that supports SNIP-9 on the target chain.
3. Ensure the session chain matches your SDK chain (`rpcUrl` + `chainId`).
...
### 1. Install Cartridge Controller (Web)

For browser apps using `starkzap`, install the web controller package:

```bash
npm install @cartridge/controller
```

For React Native with `@starkzap/native`, use a native controller.c adapter and register it once:

```typescript
import { registerCartridgeNativeAdapter } from "@starkzap/native";

registerCartridgeNativeAdapter(myControllerCAdapter);
```
...
### React Native (controller.c adapter)

In React Native, keep the same onboarding API. Register the adapter once, then call `onboard` normally:

```typescript
import { StarkZap, OnboardStrategy } from "@starkzap/native";
...
const { wallet } = await sdk.onboard({
  ...
  cartridge: {
    ...
    // Optional native-only options:
    // redirectUrl: "myapp://cartridge/callback",
...
<Warning>
  Native Cartridge support in this release is sponsored-session first. `deploy()` and `feeMode: "user_pays"` are intentionally unsupported for the native Cartridge wallet path.
</Warning>
