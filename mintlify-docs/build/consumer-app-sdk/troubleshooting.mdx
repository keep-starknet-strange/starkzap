...
description: Common issues and solutions when using the Consumer App SDK
icon: wrench
---
...
**Problem:** You're trying to call staking methods but haven't configured the staking contract.

**Solution:** Add `staking.contract` to your `StarkSDK` configuration:
...
```typescript
import { Address, Amount, ChainId } from "x";
