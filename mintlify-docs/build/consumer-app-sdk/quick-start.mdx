...
<Info>
  **For testing, use Sepolia testnet** - Replace `"mainnet"` with `"sepolia"` and use `sepoliaTokens` instead of `mainnetTokens`. The [Starknet Sepolia Faucet](https://starknet-faucet.vercel.app/) is only available for Sepolia testnet (not mainnet). On mainnet, you'll need to purchase or transfer real tokens.
</Info>

```typescript
import { StarkSDK, StarkSigner, Amount, fromAddress, sepoliaTokens } from "x";
...
const onboard = await sdk.onboard({
  ...
  privy: {
    resolve: async () => {
      // 1) Fetch/create the Privy Starknet wallet for this user from backend
      ...
      // 2) Return wallet context + signing endpoint for the SDK
      return {
        walletId: wallet.id,
      ...
      privy: {
        resolve: async () => {
          const walletRes = await fetch(
            "https://your-api.example/api/wallet/starknet",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
